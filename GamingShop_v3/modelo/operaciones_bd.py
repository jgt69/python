...
def registro_periferico(producto):
    ...
    val = (producto.tipo, producto.marca, producto.modelo, producto.gama, float(producto.precio))
...
def obtener_item_por_id(index):
    ...
    mydb.disconnect()
    objeto_periferico = Periferico(id=item_obtenido[0], tipo=item_obtenido[1], marca=item_obtenido[2], modelo=item_obtenido[3], gama=item_obtenido[4], precio=item_obtenido[5])
...
def guardar_cambios_producto(item):
    ...
    mydb = conectar()
    cursor = mydb.cursor()
    val = (item.tipo, item.marca, item.modelo, item.gama, float(item.precio), item.id)
    cursor.execute(sql, val)
    mydb.commit()
    mydb.disconnect()
